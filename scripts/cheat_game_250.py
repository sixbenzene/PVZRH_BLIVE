...
class PvzCheat:
    def __init__(self):
        ...
        self.sun_base_address = self.GameAssembly_dll+0x1D95880
        self.speedx_base_address = self.GameAssembly_dll+0x1D61E08
        self.sun_address = self.getPointerAddress(self.offset_list,self.sun_base_address) 

        # 判断左键点击
        self.mouse_click_in_address = self.GameAssembly_dll+0x3F0F3D
        # 判断右键点击
        self.mouse_right_in_address = self.GameAssembly_dll+0x3F1065
        # 点击主菜单类的判断
        self.click_menu_address = self.GameAssembly_dll+0x14DFF08
        # 访问了坐标的代码
        self.get_position_address = self.getModuleAddr("UnityPlayer.dll")+0xA1073E
        # 执行ESC的地址
        self.esc_address = self.GameAssembly_dll+0x316488 
        # 原程序记录赢的那条路
        self.win_in_address = self.GameAssembly_dll+0x49C197
        # 倍速设置
        self.speed_address = self.getPointerAddress([0xB8,0xE8],self.speedx_base_address)
        # 指定放置植物
        self.select_plants_address = self.GameAssembly_dll + 0x3E62A5
        # 指定放置僵尸
        self.select_zombie_address = self.GameAssembly_dll + 0x3E60A5
